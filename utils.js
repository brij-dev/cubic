...
RubiksCube.prototype.scramble = function(num) {
  ...
  for (var i = 0; i < num; i++) {
    ...
    const moveTomake =
      ...
      (Math.random() > 1 / 2 ? "'" : "");
    movesTaken.push(moveTomake);
    this.makeMove(moveTomake);
  }
  document.getElementById("scrambled-text").textContent = movesTaken
    .join(", ")
    .toUpperCase();
};

RubiksCube.prototype.positionView = function() {
  this.cameraX += (this.cx - this.cameraX) * 0.08;
  this.cameraY += (this.cy - this.cameraY) * 0.08;
  this.cameraZ += (this.cz - this.cameraZ) * 0.08;
  if (xAutorotate != 0) this.cx += xAutorotate;
  if (yAutorotate != 0) this.cy += yAutorotate;
  if (zAutorotate != 0) this.cz += zAutorotate;
};

RubiksCube.prototype.showCube = function() {
  this.cameraAffine = makeRotationAffine(
    this.cameraX,
    this.cameraY,
    this.cameraZ
